...
const Messages = () => {
  ...
  useEffect(() => {
    const seenHandler = ({ conversationId, userId }) => {
      // When someone marks messages as seen, reset unread count for that conversation
      setConversations((prev) =>
        prev.map((c) =>
  ...
  const handleCreateMessage = async ({ to, text }) => {
    // MessageThread will handle sending
    // Here we just reload conversations
    const convs = await getConversations();
    setConversations(convs);
  };
        ...
        {conversations.map((c) => (
          <div
            ...
            className={`p-4 cursor-pointer border-b hover:bg-gray-100 ${
              selectedConversation?.id === c.id ? "bg-gray-100" : ""
            }`}
          >
            <div className="flex items-center gap-3">
              <img
              ...
              />
              <div className="flex-1">
                <div className="font-medium">{c.partner?.name || 'Unknown User'}</div>
                <div className="text-sm text-gray-600 truncate">
