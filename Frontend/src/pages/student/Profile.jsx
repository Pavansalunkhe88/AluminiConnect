  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Show backend data ONLY if valid
        ...
        console.error("Backend unreachable → Showing dummy profile:", error);
        setProfile(dummyProfile); // fallback
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="bg-white shadow-md rounded-2xl w-[90%] md:w-[60%] overflow-hidden">
        <div className="bg-gradient-to-r from-blue-500 to-indigo-500 h-32 relative">
          <div className="absolute -bottom-10 left-10 flex items-center">
            ...
            <div className="ml-4 text-white">
              ...
              <p className="text-sm">{profile.batch ? `Class of ${profile.batch}` : ""}</p>
            </div>
          </div>
        ...
        <div className="flex justify-around border-t py-5 bg-gray-50">
          <div className="text-center">
            <p className="text-blue-600 font-bold text-lg">{profile.connections}</p>
            <p className="text-gray-600 text-sm">Connections</p>
          </div>
          <div className="text-center">
            <p className="text-green-600 font-bold text-lg">{profile.posts}</p>
            <p className="text-gray-600 text-sm">Posts</p>
          </div>
          <div className="text-center">
            <p className="text-purple-600 font-bold text-lg">{profile.profileViews}</p>
